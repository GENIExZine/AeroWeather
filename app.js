...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load initial city (Toronto or saved last city)
...
async function fetchCityWeather(cityObj) {
  ...
  try {
    ...
    const tempStr = weatherRes && weatherRes.current ? formatTemp(weatherRes.current.temperature_2m) : '';
    ...
    showToast('Failed to load real-time weather data. Please try again.', 'error');
  } finally {
...
function renderHeroCard() {
  ...
  if (locationEl) {
    ...
    locationEl.textContent = `${state.currentCity.name}, ${region}${state.currentCity.country}`;
  }
  ...
  if (bgImg && state.currentCity.photoUrl) {
    bgImg.src = state.currentCity.photoUrl;
...
/**
 * UI State Helpers (Toast & Loading Skeleton)
 */
function showLoadingState(isLoading) {
  const heroSkeleton = document.getElementById('hero-skeleton-overlay');
  if (heroSkeleton) {
    if (isLoading) heroSkeleton.classList.remove('hidden');
    else heroSkeleton.classList.add('hidden');
  }
}
